...
final class VaultListItemSearchTest extends TestCase
{
    ...
    public function testDefaultRequiresEveryTokenToMatch(): void
    {
        $row = self::row('x', 'password', 'Bank login production');
        $this->assertTrue(VaultListItemSearch::matches('production', $row, false, false));
        $this->assertTrue(VaultListItemSearch::matches('bank production', $row, false, false));
        $this->assertTrue(VaultListItemSearch::matches('production bank', $row, false, false));
        $this->assertFalse(VaultListItemSearch::matches('missing bank', $row, false, false));
        $this->assertFalse(VaultListItemSearch::matches('missing', $row, false, false));
    }

    public function testDefaultMatchesAllTokensInAnyOrderAcrossSearchableWords(): void
    {
        $row = self::row('x', 'password', 'aws rds password');
        $this->assertTrue(VaultListItemSearch::matches('aws password rds', $row, false, false));
        $this->assertTrue(VaultListItemSearch::matches('rds password aws', $row, false, false));
    }

    public function testExactWordRequiresWholePhraseInSearchable(): void
    {
    ...
    public function testExactWordRequiresContiguousPhrase(): void
    {
        $row = self::row('x', 'password', 'prod aws password rds backup');
        $this->assertTrue(VaultListItemSearch::matches('aws password rds', $row, true, false));
        $this->assertFalse(VaultListItemSearch::matches('aws password rds', self::row('x', 'password', 'aws rds password'), true, false));
    }
