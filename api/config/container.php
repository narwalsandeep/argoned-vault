...
return [
    ...
    AuthService::class => static function (\Psr\Container\ContainerInterface $c) use ($settings): AuthService {
        return new AuthService(
            ...
            (string) ($settings['legal']['signup_docs_version'] ?? '2026-04-16'),
            (string) ($settings['security']['login_otp_pepper'] ?? ''),
            (int) ($settings['security']['login_otp_ttl_seconds'] ?? 480),
        );
    },
    ...
    VaultFieldShareRepository::class => \DI\autowire(),
    VaultFieldShareRepositoryInterface::class => \DI\get(VaultFieldShareRepository::class),
    VaultRecoveryRepository::class => \DI\autowire(),
    ShareRateLimiter::class => static function (\Psr\Container\ContainerInterface $c): ShareRateLimiter {
        /** @var array<string,mixed> $settings */
        $settings = $c->get('settings');
        /** @var array<string,mixed> $shareRate */
        $shareRate = $settings['security']['share_rate_limit'] ?? [];

        return new ShareRateLimiter($shareRate, $c->get(\Psr\Log\LoggerInterface::class));
    },
    ShareRateLimiterInterface::class => \DI\get(ShareRateLimiter::class),
    VaultFieldShareService::class => static function (\Psr\Container\ContainerInterface $c): VaultFieldShareService {
        /** @var array<string,mixed> $settings */
        $settings = $c->get('settings');
        /** @var array<string,mixed> $vault */
        $vault = $settings['vault'] ?? [];

        return new VaultFieldShareService(
            $c->get(VaultFieldShareRepositoryInterface::class),
            $c->get(ShareRateLimiterInterface::class),
            ['max_active_per_user' => (int) ($vault['share_max_active_per_user'] ?? 20)],
            (string) ($settings['ui_app_base_url'] ?? 'http://localhost:4200'),
        );
    },
    ...
    BillingServiceInterface::class => static function (\Psr\Container\ContainerInterface $c): BillingServiceInterface {
        ...
        return new StripeBillingService(
            ...
            (bool) ($settings['lifetime_plan_offered'] ?? false),
        );
    },
