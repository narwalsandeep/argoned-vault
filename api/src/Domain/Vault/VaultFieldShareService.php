...
final class VaultFieldShareService
{
    ...
    public function __construct(
        private readonly VaultFieldShareRepositoryInterface $shares,
    ...
    public function fetchPublic(string $shareId, string $clientIp): ?array
    {
        ...
        $row = $this->shares->fetchActive($shareId);
        if ($row === null) {
            return null;
        }

        return $this->mapShareBlob($row);
    }

    public function consumePublic(string $shareId, string $clientIp): bool
    {
        $limit = $this->rateLimiter->consumeFetch($clientIp, $shareId);
        if (!$limit['allowed']) {
            throw new ShareRateLimitException('share_consume_rate_limited', $limit['retry_after'], $limit['storage_failed']);
        }

        $ipHash = hash('sha256', $clientIp);

        return $this->shares->consumeShare($shareId, $ipHash);
    }
     ...
     * @return array<string,mixed>
     */
    private function mapShareBlob(array $row): array
    {
