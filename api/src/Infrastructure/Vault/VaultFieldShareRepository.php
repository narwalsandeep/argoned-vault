...
final class VaultFieldShareRepository implements VaultFieldShareRepositoryInterface
{
    ...
    /**
     * Read active share ciphertext without consuming a view.
     *
     * @return array<string,mixed>|null
     */
    public function fetchActive(string $shareId): ?array
    {
        $pdo = $this->pdoFactory->create();
        $stmt = $pdo->prepare(
            'SELECT crypto_version, kdf_algo, kdf_params_json,
    ...
    /**
     * Consume one view after the client successfully decrypts. Returns false when unavailable.
     */
