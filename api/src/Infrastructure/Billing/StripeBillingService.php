...
final class StripeBillingService implements BillingServiceInterface
{
    ...
    public function __construct(
        ...
        private readonly bool $lifetimePlanOffered = false,
    ) {
        if ($this->isEnabled()) {
    ...
    public function getPublicConfig(): array
    {
        return [
            ...
            'payment_links' => [
                ...
                'lifetime' => $this->lifetimePlanOffered
                    ? (string) ($this->stripeConfig['payment_link_lifetime'] ?? '')
                    : '',
            ],
    ...
    private function applyCheckoutSession(Session $session, string $userId): void
    {
        ...
        if ($session->mode === 'payment' && $session->payment_intent !== null) {
            ...
            if ($planKey === 'lifetime' && !$this->lifetimePlanOffered) {
                $this->logger->warning('Stripe checkout ignored: lifetime plan not offered', [
                    'user_id' => $userId,
