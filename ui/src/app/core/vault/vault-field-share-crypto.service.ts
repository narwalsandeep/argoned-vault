...
export class VaultFieldShareCryptoService {
  ...
  public async encryptFieldShare(
    ...
    const aad = this.buildAad(shareId, normalizeShareExpiresAtForAad(expiresAtIso));
  ...
  public async decryptFieldShare(
    ...
    const expiresCandidates = this.aadExpiresCandidates(blob.expires_at);
    let lastError: unknown;
    for (const expiresAt of expiresCandidates) {
      try {
        const plainBuffer = await crypto.subtle.decrypt(
          {
            name: 'AES-GCM',
            iv: this.asArrayBuffer(nonce),
            additionalData: this.asArrayBuffer(this.buildAad(shareId, expiresAt)),
          },
          shareKey,
    ...
    if (lastError instanceof Error && lastError.message === 'Invalid share payload') {
      throw lastError;
    }
    throw new Error('Incorrect access code or corrupted share');
  }

  private aadExpiresCandidates(raw: string): string[] {
    const normalized = normalizeShareExpiresAtForAad(raw);
    if (normalized === raw) {
      return [raw];
    }
  ...
  private buildAad(shareId: string, expiresAtRaw: string): Uint8Array {
    const material = `${shareId}||${expiresAtRaw}||${SHARE_CRYPTO_VERSION}`;
