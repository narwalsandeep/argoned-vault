            ...
            @if (loginStep() === 'password') {
              ...
              <form [formGroup]="otpForm" (ngSubmit)="onSubmitOtp()" class="auth-form">
                <div>
                  ...
                  />
                  <p id="login-otp-hint" class="auth-info-text mt-2 text-sm">Codes expire in 2 minutes. You can request a new one below.</p>
                </div>
                <button
                  class="btn-primary auth-cta mt-1 w-full"
                  type="submit"
...
export class LoginComponent implements OnInit {
  ...
  public onResendOtp(): void {
    ...
    this.auth.resendLoginEmailOtp(this.mfaChallengeToken).subscribe({
      next: () => this.toast.success('A new code was sent to your email.'),
      error: (err) => {
        const code = err?.error?.error;
