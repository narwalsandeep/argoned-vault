...
export class VaultFileAttachmentsComponent {
  ...
  protected onFilesSelected(event: Event): void {
    ...
    if (room <= 0) {
      this.toast.error(`This file vault already has the maximum of ${VAULT_FILE_MAX_PER_VAULT_ITEM} files.`);
      inputEl.value = '';
      return;
    }
    const toUpload = batch.slice(0, room);
    if (toUpload.length < batch.length) {
      this.toast.error(`You can add at most ${room} more file(s) to this vault item (${VAULT_FILE_MAX_PER_VAULT_ITEM} total).`);
    }
  ...
  private async uploadSequentially(vaultItemId: string, picked: File[], inputEl: HTMLInputElement): Promise<void> {
    ...
    try {
      for (const selected of picked) {
        ...
        if (selected.size > VAULT_FILE_MAX_BYTES) {
          this.toast.error('Maximum file size is 20MB.');
          continue;
        }
        ...
        if (this.files().length >= VAULT_FILE_MAX_PER_VAULT_ITEM) {
          this.toast.error(`This file vault already has the maximum of ${VAULT_FILE_MAX_PER_VAULT_ITEM} files.`);
          break;
        }
        try {
