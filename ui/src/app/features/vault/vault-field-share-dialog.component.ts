...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, AppShellModalComponent, RouterLink],
  template: `
    <app-shell-modal
      [open]="open()"
      heading="Share field"
    ...
    >
      @if (createdLink() === null) {
        <form class="control-form-grid share-create-form" (ngSubmit)="onCreate()">
          ...
          <div class="control-actions-row">
            <button type="button" class="control-btn-secondary w-max" (click)="onClose()">Cancel</button>
        ...
        <div class="share-create-result control-form-grid">
          <div class="control-field">
            <label for="share-link-out">Link</label>
            <div class="control-actions-group">
              <input id="share-link-out" class="control-field min-w-0 flex-1" type="text" readonly [value]="createdLink()!" />
              <button type="button" class="control-btn-secondary" (click)="copyText(createdLink()!, 'link')">Copy</button>
            </div>
          </div>
          <div class="control-field">
            <label for="share-code-out">Access code</label>
            <div class="control-actions-group">
              <input id="share-code-out" class="control-field min-w-0 flex-1 font-mono" type="text" readonly [value]="createdCode()!" />
              <button type="button" class="control-btn-secondary" (click)="copyText(createdCode()!, 'code')">Copy</button>
            </div>
          </div>
