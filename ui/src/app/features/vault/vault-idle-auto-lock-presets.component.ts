  ...
  template: `
    ...
    >
      @for (m of minuteOptions; track m) {
        <button
          type="button"
          class="min-h-11 min-w-[3.25rem] rounded-xl border px-3 text-sm font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-app-focus focus-visible:ring-offset-2 focus-visible:ring-offset-app-bg"
          ...
          [attr.aria-label]="minuteAriaLabel(m)"
...
export class VaultIdleAutoLockPresetsComponent {
  ...
  public minuteLabel(m: number): string {
    return `${m} min`;
  }

  public minuteAriaLabel(m: number): string {
    return `${m} minutes of inactivity before auto-lock`;
  }
