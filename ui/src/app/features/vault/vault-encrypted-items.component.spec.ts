...
describe('vaultListEntryMatchesSearchTokens', () => {
  ...
  it('matches when any text token appears in searchable words or other fields', () => {
    expect(vaultListEntryMatchesSearchTokens(entry, 'production')).toBe(true);
    expect(vaultListEntryMatchesSearchTokens(entry, 'missing bank')).toBe(true);
  ...
  it('full word option requires whole phrase in searchable words', () => {
    const row = {
      id: 'x',
      display_number: 1,
      item_type: 'password',
    ...
    expect(vaultListEntryMatchesSearchTokens(row, 'alpha beta', { fullWord: true })).toBe(true);
    expect(vaultListEntryMatchesSearchTokens(row, 'beta alpha', { fullWord: true })).toBe(false);
  });
...
describe('VaultEncryptedItemsComponent', () => {
  ...
  it('defaults search full-word and match-case toggles to Off', () => {
    const fixture = TestBed.createComponent(VaultEncryptedItemsComponent);
    const c = fixture.componentInstance;
    expect(c.searchFullWord()).toBe(false);
