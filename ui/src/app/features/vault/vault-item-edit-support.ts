...
import { backfillSearchableFromDisplayNameIfEmpty, vaultDisplayNameFieldKey } from '../../core/vault/vault-searchable-words-form';
...
/** HTML `id` on inline vault edit forms; external toolbar uses `form` attribute to submit. */
export const VAULT_INLINE_EDIT_FORM_ID = 'vault-item-inline-edit-form';
...
export function patchVaultFieldsFormFromDecrypted(
  ...
  const displayNameKey = vaultDisplayNameFieldKey(fieldKeys);
  if (displayNameKey !== null) {
    backfillSearchableFromDisplayNameIfEmpty(form, displayNameKey);
  }
}
