...
import { isVaultFieldShareEligible } from '../../core/vault/vault-field-share-eligibility';
import {
  VaultFieldShareDialogComponent,
  type VaultFieldShareDialogContext,
} from './vault-field-share-dialog.component';
import {
...
@Component({
  ...
  template: `
    <div class="vault-encrypted-items-page">
                      ...
                      @for (row of decryptedDetailRows(); track row.key) {
                        ...
                        >
                          ...
                          <div class="vault-item-detail-actions">
                            ...
                            @if (canShareField(row)) {
                              <button
                                type="button"
                                class="vault-item-detail-copy vault-item-detail-share"
                                [attr.aria-label]="'Share ' + formatDetailLabel(row.key)"
                                [attr.title]="'Share ' + formatDetailLabel(row.key)"
                                (click)="openShareField(row); $event.stopPropagation()"
                              >
                                <svg class="size-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                  <path
                                    stroke-linecap="round"
                              ...
                              <span class="vault-item-detail-actions-separator" aria-hidden="true"></span>
                            }
      ...
      @if (fieldShareContext(); as shareCtx) {
        <app-vault-field-share-dialog
          [open]="fieldShareOpen()"
          [context]="shareCtx"
          (closed)="closeFieldShare()"
        />
      }
    </div>
  `,
...
export class VaultEncryptedItemsComponent {
  ...
  public readonly fieldShareOpen = signal(false);
  public readonly fieldShareContext = signal<VaultFieldShareDialogContext | null>(null);
  ...
  public canShareField(row: DecryptedDetailRow): boolean {
    return this.selectedItemDecrypted() !== null && isVaultFieldShareEligible(row.key, row.value);
  }

  public openShareField(row: DecryptedDetailRow): void {
    const entry = this.selectedEntry();
    const id = this.selectedId();
    if (entry === null || id === null) {
      return;
    }
    this.fieldShareContext.set({
      vaultItemId: id,
      itemType: entry.item_type,
      fieldKey: row.key,
      fieldLabel: this.formatDetailLabel(row.key),
      fieldValue: row.value,
    });
    this.fieldShareOpen.set(true);
  }

  public closeFieldShare(): void {
    this.fieldShareOpen.set(false);
    this.fieldShareContext.set(null);
  }
