...
describe('VaultQuickUnlockDialogComponent', () => {
  ...
  it('clears secret and refocuses input after failed unlock', async () => {
    vaultService.getProfile.mockReturnValue(of({ status: 'ok', profile: mockProfile }));
    webCryptoService.unlockVaultFromProfile.mockRejectedValue(new Error('Wrong vault unlock secret'));
    ...
    const component = fixture.componentInstance;
    component.form.patchValue({ unlock_secret: 'incorrect-secret' });
    component.submit();
    await Promise.resolve();
    await Promise.resolve();
    fixture.detectChanges();
    ...
    expect(component.form.get('unlock_secret')?.value).toBe('');
    expect(toastService.error).toHaveBeenCalledWith('Wrong vault unlock secret');
    const secret = (fixture.nativeElement as HTMLElement).querySelector(
      '#vault-quick-unlock-secret',
    ) as HTMLInputElement;
    expect(document.activeElement).toBe(secret);
  });
});
