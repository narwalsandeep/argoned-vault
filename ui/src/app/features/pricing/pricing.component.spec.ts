...
describe('PricingComponent', () => {
  ...
  it('should show loading placeholders on plan actions until billing data loads', () => {
    const cfg$ = new Subject<{ status: string; config: { enabled: boolean; payment_links: { pro: string; lifetime: string } } }>();
    const sum$ = new Subject<ReturnType<typeof summaryResponse>>();
    getConfig.mockReturnValue(cfg$.asObservable());
    getSummary.mockReturnValue(sum$.asObservable());

    const fixture = TestBed.createComponent(PricingComponent);
    fixture.detectChanges();

    const root = fixture.nativeElement as HTMLElement;
    const loaders = root.querySelectorAll('.pricing-tier-actions-loading');
    expect(loaders.length).toBe(3);
    expect(root.querySelector('.pricing-tier-actions a.btn-primary')).toBeNull();
    expect(root.querySelector('.pricing-tier-actions button.btn-primary')).toBeNull();
    ...
    cfg$.complete();
    sum$.next(summaryResponse('free'));
    sum$.complete();
    fixture.detectChanges();

    expect(root.querySelectorAll('.pricing-tier-actions-loading').length).toBe(0);
    expect(root.querySelector('.pricing-tier-actions .pricing-tier-plan-status')).toBeTruthy();
  });
  ...
  it('does not open checkout when Lifetime is hidden', () => {
